...
{
    std::cout<<"┏-------------------------------------------┓"<<std::endl;
    std::cout<<"|"<<std::right<<std::setw(10)<<"index"<<"|"<<std::right<<std::setw(10)<<"FirstName"<<"|"<<std::right<<std::setw(10)<<"LastName"<<"|"<<std::right<<std::setw(10)<<"NickName"<<"|"<<std::endl;
    std::cout<<"┗-------------------------------------------┛"<<std::endl;
    for(int i = 0; i <= 7; i++)
    {
    ...
    if(myContacts[0].GetFname() != "")
    {
        ...
        while(index == "")
        {
            ...
            {
                std::cout<<"Error! Enter a number from 1 to 8.\n";
                index = "";
            }
            else
            {
                if(myContacts[idx-1].GetFname() != "")
                {
                    idx = idx - 1;
                    std::cout<<"First Name : "<<myContacts[idx].GetFname()<<std::endl;
                    std::cout<<"Last Name : "<<myContacts[idx].GetLname()<<std::endl;
                    std::cout<<"NickName : "<<myContacts[idx].GetNname()<<std::endl;
                    std::cout<<"PhoneNumber : "<<myContacts[idx].GetPhoneNumber()<<std::endl;
                    std::cout<<"Darkest Secret : "<<myContacts[idx].Getdarkestsecret()<<std::endl;
                }
                else
                {
                    std::cout<<"Error 404, Contact NOTFOUND !"<<std::endl;
                    index = "";
                }
            }
        }
